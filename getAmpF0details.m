% This function collects selected acoustic details from .WAV files stored
% in a specific directory.  If you have excised certain .WAV files (such as
% for stimuli) you can compute acoustic details with this.  This script
% identifies the talker (c=child, f=female, m=male) coded as the first
% character of the name of the WAV file (but that could be commented out).
% For F0 estimation, this uses X. Sun's method, with his function SHRP
% appened as a subfunction below (with full reference).
% M. VanDam, 6.27.2013; 3.20.2014   labs.wsu.edu/vandam

function getAcoustDets
clear all;   
...
frame40 = rate*.04 %%% 40 ms of sample
p2 = ['*' 'o'];
%% set up the environment
currDir = pwd;
directory = input('\n\n\n\n   Enter directory where WAV files are stored: ', 's');
cd(directory);
filesWav = dir('*.wav');

%% get the acoustic observations for all the files
for j = 1:length(filesWav)
    clc;    %%% show progress
    r2 = randperm(2);
    disp([p2(r2(1)) '  working  ' p2(r2(2)) '  ' num2str(j/length(filesWav)*100) '%']);
    
    fnWav = filesWav(j).name;         %%% read in the WAV file
    cWav = wavread(fnWav);
 
    if fnWav(1) == 'c', F0MinMax = [150 800]; end
    if fnWav(1) == 'f', F0MinMax = [120 400]; end
    if fnWav(1) == 'm', F0MinMax = [50 250]; end

    [f0t, f0] = shrp(cWav,rate,F0MinMax,frame_length,...
        TimeStep,SHR_Threshold,Ceiling,med_smooth,CHECK_VOICING);
    
    [maxf0, maxi] = max(f0);          %%% get some prosody details
    [minf0, mini] = min(nonzeros(f0));

    simpF0Rise = NaN; simpF0Fall = NaN;
    if maxi > mini, simpF0Rise = 1, simpF0Fall = 0; end
    if maxi < mini, simpF0Fall = 1, simpF0Rise = 0; end
    
    
    rmsAmp = sqrt(mean(cWav .^2));  %%% RMS amplitude of whole segment
    clear cRMS;
    cRMS = [];
    for k = 1 : frame40 : length(cWav)-frame40  %% get vector of RMS intensity values of consecutive 40ms chunks
        cSeg = cWav(k:(k+frame40));
        cSegRMS = sqrt(mean(cSeg.^2));
        cRMS = [cRMS cSegRMS];
    end
    [maxA, maxA] = max(cRMS);          
    [minA, minA] = min(cRMS);
    simpArise = 0; simpAfall = 0;
    if maxA > minA, simpArise = 1; end   %% simple rising Amp over whole seg
    if maxA < minA, simpAfall = 1; end   %% simple falling Amp over whole seg
    
                                    %%% write it out to a variable
    acoust(j).fname        = fnWav;                     % file name
    acoust(j).dur          = length(cWav)/rate * 1000;  % dur in ms
    acoust(j).meanF0       = mean(nonzeros(f0));        % mean F0
    acoust(j).maxF0        = maxf0;                     % max F0
    acoust(j).minF0        = minf0;                     % min F0
    acoust(j).f0rise       = simpF0Rise;                % rising F0
    acoust(j).f0fall       = simpF0Fall;                % falling F0
    acoust(j).rmsAmp       = rmsAmp;                    % mean intensity
    acoust(j).ampRise      = simpArise;                 % rising amp
    acoust(j).ampFall      = simpAfall;                 % falling amp
end
assignin('base', 'acoustic', acoust);
%% summary of item-anaylses
clc
% noF0 = sum(isnan([acoust.meanF0]));  % segs in which F0 was not computable
f0all=[]; f0c=[]; f0f=[]; f0m=[];
fRiseAll=[]; fRiseC=[]; fRiseF=[]; fRiseM=[];
fFallAll=[]; fFallC=[]; fFallF=[]; fFallM=[];
for k = 1:length(acoust)
    if ~isnan(acoust(k).meanF0)
        f0all = [f0all, acoust(k).meanF0];
        if acoust(k).fname(1) == 'c', f0c = [f0c, acoust(k).meanF0]; end
        if acoust(k).fname(1) == 'f', f0f = [f0f, acoust(k).meanF0]; end
        if acoust(k).fname(1) == 'm', f0m = [f0m, acoust(k).meanF0]; end
    end
    if ~isnan(acoust(k).f0rise)
        fRiseAll = [fRiseAll, acoust(k).f0rise];
        if acoust(k).fname(1) == 'c', fRiseC = [fRiseC, acoust(k).f0rise]; end
        if acoust(k).fname(1) == 'f', fRiseF = [fRiseF, acoust(k).f0rise]; end
        if acoust(k).fname(1) == 'm', fRiseM = [fRiseM, acoust(k).f0rise]; end
    end
    if ~isnan(acoust(k).f0fall)
        fFallAll = [fFallAll, acoust(k).f0fall];
        if acoust(k).fname(1) == 'c', fFallC = [fFallC, acoust(k).f0fall]; end
        if acoust(k).fname(1) == 'f', fFallF = [fFallF, acoust(k).f0fall]; end
        if acoust(k).fname(1) == 'm', fFallM = [fFallM, acoust(k).f0fall]; end
    end
end


f0allOk = length(f0all)/length(acoust);
f0cOk = length(f0c)/ (length(acoust)/3);
f0fOk = length(f0f)/ (length(acoust)/3);
f0mOk = length(f0m)/ (length(acoust)/3);

disp('some summary F0 (Hz) descriptive stats:  ')
disp(['F0-all (M, SD) ' num2str(mean(f0all)), '  ' num2str(std(f0all)) '; proportion valid: ' num2str(f0allOk)])
disp(['                    proportion F0(rise)-All:  ' num2str(mean(fRiseAll))])
disp(['                    proportion F0(fall)-All:  ' num2str(mean(fFallAll))])
disp(['F0-kid (M, SD) ' num2str(mean(f0c)), '  '  num2str(std(f0c)) '; proportion valid: ' num2str(f0cOk)])
disp(['                    proportion F0(rise)-kid:  ' num2str(mean(fRiseC))])
disp(['                    proportion F0(fall)-kid:  ' num2str(mean(fFallC))])
disp(['F0-mom (M, SD) ' num2str(mean(f0f)), '  '  num2str(std(f0f)) '; proportion valid: ' num2str(f0fOk)])
disp(['                    proportion F0(rise)-mom:  ' num2str(mean(fRiseF))])
disp(['                    proportion F0(fall)-mom:  ' num2str(mean(fFallF))])
disp(['F0-dad (M, SD) ' num2str(mean(f0m)), '  '  num2str(std(f0m)) '; proportion valid: ' num2str(f0mOk)])
disp(['                    proportion F0(rise)-dad:  ' num2str(mean(fRiseM))])
disp(['                    proportion F0(fall)-dad:  ' num2str(mean(fFallM))])

%%%%%
%%%%%

rmsAll=[]; rmsC=[]; rmsF=[]; rmsM=[];
riseC=[]; riseF=[]; riseM=[];
for k = 1:length(acoust)
    rmsAll = [rmsAll, acoust(k).rmsAmp];
    if acoust(k).fname(1) == 'c'
        rmsC = [rmsC, acoust(k).rmsAmp];
        riseC = [riseC, acoust(k).ampRise];
    end
    if acoust(k).fname(1) == 'f'
        rmsF = [rmsF, acoust(k).rmsAmp]; 
        riseF = [riseF, acoust(k).ampRise];
    end
    if acoust(k).fname(1) == 'm'
        rmsM = [rmsM, acoust(k).rmsAmp]; 
        riseM = [riseM, acoust(k).ampRise];
    end
end
disp(' ')
disp('some summary amplitude (in unitless PCM vals -1:1) descriptive stats (all are valid):  ')
disp(['Amp-all (M, SD) ' num2str(mean(rmsAll)), ',  ' num2str(std(rmsAll))])
disp(['Amp-kid (M, SD) ' num2str(mean(rmsC)), ',  '  num2str(std(rmsC))])
disp(['Amp-mom (M, SD) ' num2str(mean(rmsF)), ',  '  num2str(std(rmsF))])
disp(['Amp-dad (M, SD) ' num2str(mean(rmsM)), ',  '  num2str(std(rmsM))])
disp(['proportion Amp(rise)-All:   ' num2str(mean([acoust.ampRise]))])
disp(['proportion Amp(rise)-kid:   ' num2str(mean(riseC))])
disp(['proportion Amp(rise)-mom:   ' num2str(mean(riseF))])
disp(['proportion Amp(rise)-dad:   ' num2str(mean(riseM))])

%%%%%
%%%%%
durAll=[]; durC=[]; durF=[]; durM=[];
for k = 1:length(acoust)
    durAll = [durAll, acoust(k).dur];
    if acoust(k).fname(1) == 'c'
        durC = [durC, acoust(k).dur];
    end
    if acoust(k).fname(1) == 'f'
        durF = [durF, acoust(k).dur]; 
    end
    if acoust(k).fname(1) == 'm'
        durM = [durM, acoust(k).dur]; 
    end
end

disp(' ')
disp('some summary duration (ms) descriptive stats (all are valid):  ')
disp(['Duration-all (M, SD) ' num2str(mean(durAll)), ',  ' num2str(std(durAll))])
disp(['Duration-kid (M, SD) ' num2str(mean(durC)), ',  '  num2str(std(durC))])
disp(['Duration-mom (M, SD) ' num2str(mean(durF)), ',  '  num2str(std(durF))])
disp(['Duration-dad (M, SD) ' num2str(mean(durM)), ',  '  num2str(std(durM))])
